...
export const MessageState = ({ messageState }: MessageStateProps) => {
  ...
  return (
    <motion.div animate={controls} className='flex flex-col'>
      <Lottie
        className={classNames('w-4 h-4 duration-300', animationState === 1 ? 'opacity-100' : 'opacity-0')}
        animationData={clockAnimationData}
        loop
      />
      <IoCheckmark
        className={classNames('w-4 h-4 duration-300', animationState === 2 ? 'opacity-100' : 'opacity-0')}
        color='8A8A8A'
      />
      <IoCheckmarkDone
        className={classNames('w-4 h-4 duration-300', animationState === 3 ? 'opacity-100' : 'opacity-0')}
